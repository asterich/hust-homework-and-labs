...
static void touch_event_cb(int fd) {
  ...
  case TOUCH_PRESS:
    ...
    if (status.start) { //游戏开始状态
      fb_draw_rect(p_board->x - board_len / 2, board_height, board_len, 10,
                   FB_COLOR(255, 255, 255)); //清除上次的
      if (x <= 62)
        x = 62;
      else if (x + 60 > SCREEN_WIDTH)
        x = 962;
      fb_draw_rect(x - board_len / 2, board_height, board_len, 10,
                   FB_COLOR(254, 67, 101)); //画出自己的木板
      p_board->x = x;
      //客户端仅传递自身板的位置
      t[0] = p_board->x;
...
static void move_event_cb(int fd) {
  if (status.start) {
    fb_draw_circle(p_ball->x, p_ball->y, p_ball->radius,
                   FB_COLOR(255, 255, 255));
    printf("moving\n");
    move(p_ball, p_status, p_board, op_p_board);
    fb_draw_circle(p_ball->x, p_ball->y, p_ball->radius,
                   FB_COLOR(128, 128, 128));
    fb_update();
  }
}
...
static void bluetooth_tty_event_cb(int fd) {
  ...
  if (n <= 0) {
    ...
    case 1:
      ...
      p_ball->radius = 10;
      fb_draw_circle(p_ball->x, p_ball->y, p_ball->radius,
                     FB_COLOR(128, 128, 128)); //画出球的初始位置
      fb_draw_rect(t[1] - board_len / 2, board_height, board_len, 10,
                   FB_COLOR(30, 144, 255)); // 画出服务端的木板
      break;
    case 2:
      //服务端木板移动
      op_p_board->x = t[1];
...
int main(int argc, char *argv[]) {
  ...
  fb_draw_text(445, 305, "Start!", 32, FB_COLOR(0, 0, 0));
  fb_update();
  //打开多点触摸设备文件, 返回文件fd
