...
void move(ball_info *p, status_info *ps, position *pb, position *p_op) {
  ...
  //碰撞上边界
    ...
    //碰撞挡板
    if (p->y + p->radius >= board_height - 2 &&
        (p->x >= pb->x - board_len / 2 && p->x < pb->x + board_len / 2)) {
      ps->collision = 1;
      //左半边
      if (p->x < pb->x - 25) {
        p->dx -= 10;
      ...
      ps->count += 1;
    } else if (p->y + p->radius >= board_height - 2 &&
               (p->x >= p_op->x - board_len / 2 &&
                p->x < p_op->x + board_len / 2)) {
      ps->collision = 1;
      //左半边
      if (p->x < p_op->x - 25) {
...
void init_board(position *p) {
  p->x = rand() % 904 + 60;
  fb_draw_rect(p->x - board_len / 2, board_height, board_len, 10,
               FB_COLOR(30, 144, 255));
  return;
}
